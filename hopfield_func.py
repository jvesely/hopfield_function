...
class HopfieldFunc(HopfieldNetwork):
    def __init__(self, domain_bits, range_bits, iterations = 100, pad_zeros = False):
    ...
    def get(self, x):
        bin_x = np.array([int(x) for x in bin(x)[2:]])
        padding = n.zeros(self.__range_bits) if self.__pad_zeros else np.random.randint(0, 1, self.__range_bits)
